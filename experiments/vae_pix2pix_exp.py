...
class Pix2pixExperiment(pl.LightningModule):
    ...
    def __init__(self, gen_model,disc_model,vae_model,
                 params: dict) -> None:
    ...
    def training_step(self, batch, batch_idx, optimizer_idx = 0):
        input_img, dem,_ = batch
        self.curr_device = input_img.device
        with torch.no_grad():
            mu = self.vae_model.encode(input_img)[0]
            res = self.vae_model.decode(mu)
        # res = (res*2)-1
        
        if optimizer_idx==0:
            self.gen_output = self.forward(res)
            with torch.no_grad():
                disc_gen_output = self.disc_model(res, self.gen_output)
            ...
            disc_real_output = self.disc_model(res, dem)
            disc_gen_output = self.disc_model(res, self.gen_output.detach())
    ...
    def validation_step(self, batch, batch_idx ):
        ...
        # res = self.vae_model(input_img)[0]
        ...
        gen_output = self.forward(res)
        disc_gen_output = self.disc_model(res, gen_output)
        disc_real_output = self.disc_model(res, dem)
    ...
    def sample_images(self):
        ...
        test_input, test_label,_ = next(iter(self.sample_dataloader))
